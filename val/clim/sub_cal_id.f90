...
subroutine cal_idlon(im1,lon1,im2,lon2,id)
  ...
  do i2=1,im2
     ...
     if(lon1(im1)-n1*360.d0 <= lon2(i2)-(n2-1)*360.d0 &
          & .and. lon2(i2)-(n2-1)*360.d0 <= lon1(1)-(n1-1)*360.d0)then
        id(i2)=i1
        cycle
     end if     
     ...
     do i1=1,im1-1
        n1=floor(lon1(i1)/360.d0)
        if(lon1(i1)-n1*360.d0 <= lon2(i2)-n2*360.d0 &
             & .and. lon2(i2)-n2*360.d0 <= lon1(i1+1)-n1*360.d0)then
           id(i2)=i1
           exit
        end if
     end do !i1

...
#For Data conversion
set range1=0.5/359.5/-89.5/89.5
set range2=0.5/359.5/-89.5/89.5
set int1=1/1
set int2=1/1
...
set depth=(0)
#@ ndep=6
#set depth=(0 100 500 1000 2500 5000)
...
#Salinity
gmt makecpt -T30/36/0.5 -CbatlowK -D > s_color.cpt
gmt makecpt -T20/40/0.5 -Croma -D -I > s_contour1.cpt
...
while($idep <= $ndep)
    ...
    echo "Make figure..."
    gmt begin fig/${var}_${depth[$idep]}m png

    @ idat = 1
    while($idat <= $ndat)

	echo "Data conversion..."
	set input=dat/${datname[$idat]}_${var}.dat
	if($idat == 1)then
	    gawk -v z=${depth[$idep]} '{if($3 == z && $5 != -999) print $1,$2,$5 > "obs.20"}' ${input} &
	else
	    gawk -v z=${depth[$idep]} '{if($3 == z && $4 != -999) print $1,$2,$4 > "dat.20"}' ${input} &
	    gawk -v z=${depth[$idep]} '{if($3 == z && $4 != -999 && $5 != -999) print $1,$2,$4-$5 > "dif.20"}' ${input} &
	endif
	wait

	if(-f obs.20) gmt surface obs.20 -Gobs.grd -R${range1} -I${int1} -T0.50 -fg
	if(-f dat.20) gmt surface dat.20 -Gdat.grd -R${range1} -I${int1} -T0.50 -fg
	if(-f dif.20) gmt surface dif.20 -Gdif.grd -R${range1} -I${int1} -T0.50 -fg
	wait
	...
	if($idat == 1)then
	    ...
	    gmt psmask obs.20 -R${range2} -I${int1}
	    #gmt grdview obs.grd -C${color}.cpt -Qs
	    gmt grdimage obs.grd -C${color}.cpt

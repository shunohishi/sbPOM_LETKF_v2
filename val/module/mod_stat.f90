...
contains
  ...
  subroutine paired_t_test(nall,dat1,dat2,dof,tcrit,tval)
    ...
    real(kind=8), parameter :: tcrit_table(ndof) = (/ &
         ...
         2.627d0, 2.626d0, 2.626d0, 2.626d0, 2.626d0  /)  ! df=96-100
...
!         &  2.013d0, 2.012d0, 2.011d0, 2.010d0, 2.009d0 /)   ! df=46-50
    ...
    !Effective degree of freedom (Breetherton et al. 1999)
    if(abs(cor) == 1.d0)then
       return
    end if
    ...
    write(*,*) "Lag-correlaction:",cor
    write(*,*) "Effective degree of freedom:",dof
    write(*,*) "T value:",tval

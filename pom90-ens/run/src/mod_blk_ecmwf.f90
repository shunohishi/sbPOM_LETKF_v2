...
CONTAINS
   ...
   FUNCTION psi_m_ecmwf( pzeta )
      ...
      !$omp parallel
      !$omp do private(jj,ji,zzeta,zx,ztmp,psi_unst,psi_stab,stab)      
      DO jj = 1, jpj
         DO ji = 1, jpi
            !
   ...
   FUNCTION psi_h_ecmwf( pzeta )
      ...
      !$omp parallel
      !$omp do private(jj,ji,zzeta,zx,psi_unst,psi_stab,stab)      
      DO jj = 1, jpj
         DO ji = 1, jpi
            !
            zzeta = MIN(pzeta(ji,jj) , 5.)   ! Very stable conditions (L positif and big!):
   ...
   SUBROUTINE CSWL_ECMWF( pQsw, pQnsol, pustar, pSST, pTs )
      ...
      !$omp parallel
      !$omp do private(jj,ji,zQnsol,zlamb,zdelta,zfs,zQnet)      
      DO jj = 1, jpj
         DO ji = 1, jpi
            !
      ...
      DO jwl = 1, nbi  ! itteration to solve implicitely equation for warm layer
         ...
         !$omp parallel
         !$omp do private(jj,ji,ZDSST,ZSRD,zsgn,ztmp,zdl,zus_a,zdL2,zPHI,ZZ)
         DO jj = 1, jpj
            DO ji = 1, jpi
